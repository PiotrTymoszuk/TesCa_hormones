...
# Similarity -------
  ...
  tcga_xcell$similarity$red_obj <- tcga_xcell$analysis_tbl %>% 
    column_to_rownames('ID') %>% 
    select(-class) %>% 
    center_data('median') %>% 
  ...
  tcga_xcell$similarity$plots <- 
    plot_similarity(tcga_xcell$similarity$red_obj, 
                    class_assignment = tcga_xcell$analysis_tbl[c('ID', 'class')], 
                    plot_title = 'Similarity of hormonal subsets, xCell infiltration', 
